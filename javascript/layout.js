...
var dataLGA
var dataLGAOne;
var countiesGraphicsLayerEnabled;
...
function createMap(webmapitem) {
    ...
    mapDeferred.addCallback(function (response) {
        ...
        if (configOptions.displaysearch === true) {
            ...
            //esri.hide(dojo.byId('webmap-toolbar-right')); //bn
        }
        addHelpLink();
...
function initUI(response) {    
    ...
    if (displayLeftPanel()) {
        ...
        if (configOptions.displaydetails === true ) {
            ...
            var detailTb = new dijit.form.ToggleButton({
                showLabel: true,
                label: "TOC", //i18n.tools.details.label,
                title: "TOC", //i18n.tools.details.title,
                checked: true,
    ...
    createZoomtoRegionTool();


    countiesGraphicsLayer = new esri.layers.GraphicsLayer();
    map.addLayer(countiesGraphicsLayer);

    //Added Aboriginal Layer ID and 'ON' event   
        map._layers.Aboriginal_Communities_942.on("mouse-over", showTooltip);
        map._layers.Aboriginal_Communities_942.on("mouse-out", closeDialog);

        var serviceProviderLayer = map._layers.ServiceProviders_5027;
        var abCommunityLayer = map._layers.Aboriginal_Communities_942;

    //Added Service Provider Layer ID and 'ON' event   
        map._layers.ServiceProviders_5027.on("mouse-over", showTooltipSP);
        map._layers.ServiceProviders_5027.on("mouse-out", closeDialogSP);
        //debugger;
        //map._layers.SP_COMM.on("mouse-over", showTooltipSPCom);
        //map._layers.SP_COMM.on("mouse-out", closeDialogSPCom);


        dojo.connect(abCommunityLayer, "onClick", function (evt) {
            enableSpiralMapabComm(evt.graphic.attributes.CommunityName);
            map.disableDoubleClickZoom();
        });
    

        dojo.connect(serviceProviderLayer, "onClick", function (evt) {
            //resetMap();
            map.graphics.disableMouseEvents();
            enableSpiralMap(evt.graphic.attributes.NameOfServiceProvider);
            map.disableDoubleClickZoom();
        });
...
function addHelpLink() {
    ...
    dojo.byId('webmap-toolbar-right').appendChild(helpButton.domNode);
    dojo.connect(helpButton, 'onClick', function () {
        window.open("User Instructions updated.htm", '_blank');
    });
}

function addRefreshMap() {
    ...
    dojo.byId('webmap-toolbar-center').appendChild(refreshButton.domNode);
...
function showTooltip(evt) {
    closeDialog();
    var tipContent = "<b>Community Name</b>: " + evt.graphic.attributes.CommunityName +
     "<br><b>Alternate Names</b>: " + evt.graphic.attributes.AlternateNames +
     "<br><b>Population</b>: " + evt.graphic.attributes.Population;

    var dialog = new dijit.TooltipDialog({
        id: "tooltipDialog",
        content: tipContent,
        style: "position: absolute; width: 250px; font: normal normal  9pt Tahoma;z-index:100"
    });
    dialog.startup();
...
function showTooltipSP(evt) {
    closeDialogSP();
    var tipContent = "<b>Service Provider </b>: " + evt.graphic.attributes.NameOfServiceProvider+
      "<br><b>Services </b>: " + evt.graphic.attributes.ServicesProvided +
      "<br><b>Phone</b>: " + evt.graphic.attributes.Phone;
...
function closeDialogSP() {
    var widget = dijit.byId("tooltipDialogSP");
    if (widget) {
        widget.destroy();
    }
}

function showTooltipSPCom(evt) {
    closeDialogSPCom();
    var tipContentSPC = "<b>Service(s) Provided </b>: " + evt.graphic.attributes.ServicesPr +
      "<br><b>Community </b>: " + evt.graphic.attributes.CommunityN;

    var dialogSPC = new dijit.TooltipDialog({
        id: "tooltipDialogSPCom",
        content: tipContentSPC,
        style: "position: absolute; width: 250px; font: normal normal  9pt Tahoma;z-index:100"
    });
    dialogSPC.startup();
    dijit.placeOnScreen(dialogSPC.domNode, { x: evt.pageX, y: evt.pageY }, ["TL", "BL"], { x: 10, y: 10 });

}

function closeDialogSPCom() {
    var widget = dijit.byId("tooltipDialogSPCom");
    if (widget) {
        widget.destroy();
    }
}
...
function enableSpiralMap(orgName) {  

    var queryTask = new esri.tasks.QueryTask("http://services2.arcgis.com/tnYxyDVKNn2b7hiw/ArcGIS/rest/services/SP_COMM/FeatureServer/0");
    var query = new esri.tasks.Query();
    query.returnGeometry = true;
    query.outFields = ["NameOfServ", "ServicesPr", "CommunityN"];
    query.where = "NameOfServ = '" + orgName + "'";

    var infoTemplate = new esri.InfoTemplate();
    infoTemplate.setTitle("${NameOfServ}");
    infoTemplate.setContent("<b>Services Provided: </b>${ServicesPr}<br/>"
                         + "<b>Community: </b>${CommunityN}<br/>");

    dojo.connect(queryTask, "onComplete", function(featureSet) {
        
        var symbol = new esri.symbol.SimpleLineSymbol("SHORTDASH", new dojo.Color([128, 0, 128]), 5);
        var highlightSymbol = new esri.symbol.SimpleLineSymbol("SOLID", new dojo.Color([0, 128, 255]), 1);
...
function enableSpiralMapabComm(communityName) {
    ...
    var query = new esri.tasks.Query();
    query.returnGeometry = true;
    query.outFields = ["CommunityN", "ServicesPr", "NameOfServ"];
    query.where = "CommunityN = '" + communityName + "'";

    var infoTemplate = new esri.InfoTemplate();
    infoTemplate.setTitle("${CommunityN}");
    infoTemplate.setContent("<b>Services: </b>${ServicesPr}<br/>"
                         + "<b>Service Provider: </b>${NameOfServ}<br/>");

    dojo.connect(queryTask, "onComplete", function (featureSet) {



        var symbol = new esri.symbol.SimpleLineSymbol("SHORTDASH", new dojo.Color([255, 128, 0]), 5);
        var highlightSymbol = new esri.symbol.SimpleLineSymbol("SOLID", new dojo.Color([128,0,64]), 1);
...
function addLayerList(layers) { 
    ...
    if (layerList.length > 0) {
        ...
        dojo.forEach(layerList, function (layer) {
            ...
            lbl = dojo.create("label", { innerHTML: layer.title, "for": chkboxName }, widgetNode);
            dojo.place("<br/>", widgetNode);            
            dojo.style(chk, {
                "float":"left",
                "display":"block",
                "padding":"0",
            ...
            dojo.place(widgetNode, dojo.byId('detailPanel'));
...
function createZoomtoRegionTool() {
    ...
    //$.ajax({
    //    url: "ZoomtoRegion.csv",
    //    aync: false,
    //    success: function (csvVals) {
    //        dataRegions = $.csv.toObjects(csvVals);
    //    },
    //    dataType: "text",
    //    complete: function () {

    //        for (var i = 0; i < dataRegions.length; i++) {
    //            col.push(dataRegions[i].Name);
    //            var extent = new esri.geometry.Extent({
    //                "xmin": parseFloat(dataRegions[i].xmin), "ymin": parseFloat(dataRegions[i].ymin), "xmax": parseFloat(dataRegions[i].xmax), "ymax": parseFloat(dataRegions[i].ymax),
    //                "spatialReference": map.spatialReference
    //            });
    //            menu.addChild(new dijit.MenuItem({
    //                label: dataRegions[i].Name,
    //                //iconClass: "emailIcon",
    //                //disabled: true,
    //                onClick: function () {
    //                    //alert('hi');
    ...
    var extent = new esri.geometry.Extent({
        "xmin": 14655920.6482, "ymin": -1703192.7277, "xmax": 14771448.1575, "ymax": -1595353.2001,
        "spatialReference": map.spatialReference
    });
    var extentB = new esri.geometry.Extent({
            "xmin":14634969.8426 ,"ymin": -2606303.0593, "xmax": 15385964.6779 , "ymax": -1843454.8669,
            "spatialReference": map.spatialReference
    });
        //var menu = new dijit.Menu({
        //    id: 'showRegionMenu'
        ...
        menu.addChild(new dijit.MenuItem({
            ...
            onClick: function () {
                //alert('hi');
                map.setExtent(extentB.expand(2));
            }
        }));

        var regionButton = new dijit.form.DropDownButton({
            label: 'Select Region',
            id: 'selectRegion',
            title: 'selectRegion',
            dropDown: menu
        ...
        regionButton.startup();
        dojo.byId('webmap-toolbar-center').appendChild(regionButton.domNode);
...
function createSearchTool() {
    ...
    $("#webmap-toolbar-right").append("<input id='tags' class='textbox' type='text'/>");        

        var column = [];

        $.ajax({
            url: "AutoComplete_Dat.csv",
            aync: false,
            success: function (csvVals) {
                dataLGA = $.csv.toObjects(csvVals);
            },
            dataType: "text",
            complete: function () {
                
                for (var i = 0; i < dataLGA.length; i++) {
                    column.push(dataLGA[i].NameofOrganisation);
                }
        ...
        $("#tags").autocomplete({
            source: function (request, response) {
                var matches = $.map(column, function (acItem) {
...
function zoomSearch(locName) {

    this.map.graphics.clear();

    var wanted = $.grep(dataLGA, function (n, i) {
        return n.NameofOrganisation == locName;
    });



    var point = new esri.geometry.Point({
        latitude: wanted[0].Latitude,
        longitude: wanted[0].Longitude
    });


    map.infoWindow.clearFeatures();
    map.infoWindow.setContent(locName);  
    ...
    var chartLocationSymbol = new esri.symbol.SimpleMarkerSymbol(esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE, 15, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID,
        new dojo.Color([0, 0, 255, 0.5]), 8), new dojo.Color([0, 0, 255]));

    this.chartLocationGraphic = new esri.Graphic(point, chartLocationSymbol);
